@class NSObject, NSString, NSArray, NSDictionary, UIView, UIViewController; 
@protocol NSObject, NSCopying, NSCoding;
//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.672, BuildCommit: 400d3d0bcbb331f43a3266dd71d67c779f12e7d8)
//
//    - LC_BUILD_VERSION:  Platform: iOS, MinOS: 16, SDK: 26.2, Tool: ld (1230.1)
//    - LC_SOURCE_VERSION: 0.0.0.0.0
//
#ifndef LOTMaskNodeLayer_h
#define LOTMaskNodeLayer_h
@import Foundation;

#include "LOTMask.h"
#include "LOTNumberInterpolator.h"
#include "LOTPathInterpolator.h"

@class CAShapeLayer;

@interface LOTMaskNodeLayer : CAShapeLayer {
    /* instance variables */
    LOTPathInterpolator *_pathInterpolator;
    LOTNumberInterpolator *_opacityInterpolator;
    LOTNumberInterpolator *_expansionInterpolator;
}

@property (readonly, nonatomic) LOTMask *maskNode;

/* instance methods */
- (id)initWithMask:(id)mask;
- (void)updateForFrame:(id)frame withViewBounds:(struct CGRect { struct CGPoint { double x0; double x1; } x0; struct CGSize { double x0; double x1; } x1; })bounds;
- (_Bool)hasUpdateForFrame:(id)frame;

@end

#endif /* LOTMaskNodeLayer_h */
