  @class NSObject, NSString, NSArray, NSDictionary, UIView, UIViewController; 
  @protocol NSObject, NSCopying, NSCoding;
//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.672, BuildCommit: 400d3d0bcbb331f43a3266dd71d67c779f12e7d8)
//
//    - LC_BUILD_VERSION:  Platform: iOS, MinOS: 16, SDK: 26.2, Tool: ld (1230.1)
//    - LC_SOURCE_VERSION: 0.0.0.0.0
//
#ifndef PINAnimatedImage_h
#define PINAnimatedImage_h
@import Foundation;

@class NSObject;

@interface PINAnimatedImage : NSObject

@property (readonly, nonatomic) unsigned long long frameCount;
@property (readonly, nonatomic) double totalDuration;
@property (readonly, nonatomic) unsigned long long frameInterval;

/* class methods */
+ (long long)maximumFramesPerSecond;
+ (unsigned long long)greatestCommonDivisorOfA:(unsigned long long)a andB:(unsigned long long)b;

/* instance methods */
- (id)init;
- (double)durationAtIndex:(unsigned long long)index;
- (double)minimumFrameInterval;

@end

#endif /* PINAnimatedImage_h */
